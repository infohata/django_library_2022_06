...
def register(request):
    if request.method == "POST":
        ...
        if not username or get_user_model().objects.filter(username=username).exists():
            ...
            # Vartotojo vardas neįvestas, arba toks vartotojas jau egzistuoja.
            is_error = True
        if not email or get_user_model().objects.filter(email=email).exists():
            ...
            # El.pašto adresas neįvestas, arba vartotojas su įvestu el.pašto adresu jau egzistuoja.
            is_error = True
        if not password or not password2 or password != password2:
            ...
            is_error = True
            # Slaptažodžiai nesutampa arba neįvesti.
        try:
